  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section aria-label="Careers hero" className="relative pt-32 pb-20 md:pt-48 md:pb-32 overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-slate-50 via-slate-50/30 to-white -z-10" aria-hidden="true" />
        ...
        <div className="container" id="main-content">
          <motion.div className="max-w-4xl" initial="hidden" animate="visible" variants={containerVariants}>
            ...
            >
              ...
              <span className="text-sm font-bold text-[#1B3A4B]">Join Our Team</span>
            </motion.div>
        ...
        <div className="container">
          ...
          {isLoading && (
            <div className="flex justify-center py-20" role="status" aria-label="Loading job listings">
              <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[#1B3A4B]" aria-hidden="true" />
            ...
            >
              {jobs.map((job) => (
                <motion.li
                  ...
                  className="group p-8 bg-white rounded-2xl border border-slate-200 hover:border-[#B8973E]/50 hover:shadow-xl transition-all flex flex-col h-full"
                  whileHover={{ y: -5 }}
                >
                  <div className="flex justify-between items-start mb-6">
                    <Briefcase className="w-10 h-10 text-[#1B3A4B]" aria-hidden="true" />
                    <span className="text-sm font-bold text-[#1B3A4B] bg-[#EEF2F5] px-3 py-1 rounded-full">
                      {job.type}
                    </span>
                  </div>
                  ...
                  <Link href={`/job/${job.id}`}>
                    <a
                      className="mt-auto inline-flex items-center gap-2 text-[#1B3A4B] font-bold hover:gap-3 transition-all focus:outline-none focus:underline"
